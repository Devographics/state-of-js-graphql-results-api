...
export default {
    ToolsRankings: {
        experience: async (rankings, args, context, info) => {
            return await getCachedResult(computeToolsExperienceRanking, context.db, [
                rankings.ids,
                rankings.survey
            ])
        }
    ...
    ToolExperience: {
        allYears: async (tool, args, context, info) => {
            ...
            return await getCachedResult(computeExperienceOverYears, context.db, [
                tool.id,
                tool.survey
            ])
        },
        year: async (tool, args, context, info) => {
            ...
            const allYears = await getCachedResult(computeExperienceOverYears, context.db, [
                tool.id,
                tool.survey
            ])
